...
# Number of processes = number of GPUs if available, else 2
NUM_GPUS = torch.cuda.device_count()
#NUM_PROCESSES = NUM_GPUS if NUM_GPUS > 0 else 2
NUM_PROCESSES = 2
...
def process_chunk(chunk, step_path, chunk_id):
    ...
    for f in tqdm(chunk, desc=desc_chunk, position=chunk_id):
        ...
        depths = []
        for i in tqdm(range(0, len(images), BATCH_SIZE), desc=f"File {f}", position=chunk_id, leave=False):
